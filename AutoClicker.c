...
int main() {
    ...
    set_console_color(FOREGROUND_GREEN); printf("JohanLibert\n"); reset_console_color();
    printf("\n--------------------------------------------\n");
    printf("Auto-Clicker (C Version: Smart & Human-like - RIGHT CLICK)\n");
    printf("--------------------------------------------\n\n");
    ...
    printf("Controls:\n");
    printf("  - Press 'B' to activate/deactivate.\n");
    printf("  - Hold 'Y' to temporarily disable.\n");
    printf("  - Hold RMB (0.25s) for Mining Mode.\n");
            ...
            if (!lmb_was_held_long_enough) { // Only check if we haven't already entered mining mode
                ...
                if (elapsed_ms >= HOLD_TO_DEACTIVATE_MS) {
                    if (running) { // Only deactivate if it was running
                        ...
                        display_status_message("Mining Mode active (RMB held).", FOREGROUND_INTENSITY | FOREGROUND_RED);
